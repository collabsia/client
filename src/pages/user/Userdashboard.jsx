...
const Userdashboard = () => {
  ...
  const handleDateClick = async (value) => {
    ...
    try {
      ...
      const [memoResponse, eventResponse] = await Promise.all([
        axios.post('/api/memo/send-and-recieve', { date: formattedDate, token }, { headers: { Authorization: `Bearer ${token}` } }),
        axios.post('/api/getevent', { token, date: formattedDate }, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const handleEventClick = async (event, memoId) => {
    try {
      await axios.post('/api/memo/read', { token, memoId }, { headers: { Authorization: `Bearer ${token}` } });
